/**
 * Gen-6+ type effectiveness chart (unchanged through gen 9 / Champions).
 * Only non-neutral matchups are listed; anything absent is ×1.
 */
export const TYPES = [
...
const norm = (t: string): TypeName => (t[0].toUpperCase() + t.slice(1).toLowerCase()) as TypeName;

/** Multiplier of an attacking type into a defender's type(s). */
export function effectiveness(attacking: string, defending: string[]): number {
  const atk = norm(attacking);
  return defending.reduce((mult, d) => mult * (CHART[atk]?.[norm(d)] ?? 1), 1);
}

/** All type matchups against a defender: { Fire: 2, Water: 0.5, ... } */
export function defensiveProfile(defending: string[]): Record<TypeName, number> {
  const out = {} as Record<TypeName, number>;
  for (const t of TYPES) out[t] = effectiveness(t, defending);
  return out;
}
